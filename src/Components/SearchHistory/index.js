...
class SearchHistory extends Component {
  ...
  deleteHistory = id => {
    ...
    const searchResults = historyDetails.filter(eachItem => eachItem.id !== id)
    this.setState({historyDetails: searchResults})
  }

  render() {
    const {searchInput, historyDetails} = this.state

    const filteredSearchResults = historyDetails.filter(eachItem =>
      eachItem.title.includes(searchInput),
    )
    let r = false

    if (historyDetails.length === 0 || filteredSearchResults.length === 0) {
      r = true
    } else {
      r = false
    }

    const msg = r ? 'There is no history to show' : null
